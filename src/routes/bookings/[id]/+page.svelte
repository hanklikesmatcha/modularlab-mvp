	...
	<div class="card card-side bg-base-100 shadow-xl max-w-screen-md">
		<div class="card-body">
			<div class="flex flex-row justify-between">
				<h2 class="card-title">
					{booking.suppliers.name} - {new Date(
						booking.created_at
					).toLocaleDateString()}
				</h2>
				<div class="radial-progress" style="--value:0;  --thickness: 6px">
					0%
				</div>
			</div>
					...
					<tbody class="text-base">
						<tr>
							...
							<td>
								<label
									for="drawing-modal"
									class="btn btn-primary btn-xs btn-outline btn-bg-neutral"
								...
								<div class="modal modal-bottom sm:modal-middle">
									<div class="flex justify-center  modal-box">
										<label
											for="drawing-modal"
											class="btn btn-sm btn-circle btn-outline border-hidden absolute right-2 top-2"
											>✕</label
										>
										{#if booking.fire_url}
											<figure class="m-6 w-full rounded-sm">
												<img src={booking.file_url} alt="supplier" />
											</figure>
										{:else}
